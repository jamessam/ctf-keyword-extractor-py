...
class UnitTests(TestCase):
    ...
    def test_get_keywords(self):
        ...
        keywords = get_keywords('test_files/StockSnap_ICUFI7PZGT.jpg')
        self.assertEqual(expected, keywords)
        expected = []
        keywords = get_keywords('test_files/StockSnap_PPBG50NAWP.jpg')
        self.assertEqual(expected, keywords)
        expected = ['baseball', 'striking image', 'ball']
        keywords = get_keywords('test_files/StockSnap_TXQJEEV01S.jpg')
        self.assertEqual(expected, keywords)
